#!/usr/bin/env python3
"""
Recriar imagens 1x corretamente mantendo proporções originais
"""
...
for img_path in images:
    ...
    # Calcular nova largura máxima (600px) mantendo aspect ratio
    max_width = 600
    ...
    # Salvar como WebP 1x
    output_name = img_path.replace('.jpg', '_opt.webp')
    img_resized.save(output_name, 'WEBP', quality=75, method=6)

    size = os.path.getsize(output_name) / 1024
    print(f"   Criado: {output_name} - {size:.1f} KB")

print("\nImagens 1x recriadas corretamente!")
